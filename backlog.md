...
- [ ] [P2] `AIW-20` **Update hardcover cover on KDP/Amazon**: Subtitle repositioned (moved up 0.25in toward title to avoid overlap with image). Rebuild `cover-wrap-hc.pdf` and upload to KDP. Check paperback cover (`cover-wrap.tex`) and EU variants for same issue.
- [ ] [P2] `AIW-21` **German book revision — content changes**: Three issues to address in `pop-sci/book-manuscript-de.md`: (1) **No drug self-reports** — any first-person accounts of drug experiences must be rewritten as third-party accounts or general descriptions; (2) **Less about the author** — reduce autobiographical content; (3) **Bernhard Glück section** — remove specific examples of mutual comedic criticism, keep the relationship but cut the illustrative anecdotes.
