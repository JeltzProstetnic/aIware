...
- [ ] [P2] `AIW-06` **Conference submissions still open**: 6ICPH Porto (rolling, 250-word abstract, hybrid), SAGE Adaptive Behavior neurophenomenology (May 1, 2026), Models of Consciousness 7 Shanghai (TBA, monitor amcs-community.org).
...
- [ ] [P3] `AIW-08` **arXiv preprint** (AFTER NoC decision): Needs endorsement (q-bio.NC). Endorsement link: https://arxiv.org/auth/endorse?x=E9JU9T. Submit BEFORE journal (65% citation boost). Requires syncing arxiv/paper.tex with current paper content.
...
- [ ] [P3] `AIW-11` **Pop-sci book — review pass**: English `pop-sci/book-manuscript.md` (~31,000 words, needs flow/consistency review). German `pop-sci/book-manuscript-de.md` (~2,400 lines, quality revision ~75% complete). KDP setup complete (ISBNs assigned, covers built).
- [ ] [P3] `AIW-15` **Redesign "simple" diagrams in all AC design PDFs**: The current simple diagrams are too abstract. Redesign them to match the clarity level of the mirror-box README ASCII diagram — the most human-readable version. Same PDF color scheme, same Mermaid rendering pipeline (`mmdc` → PNG → WeasyPrint), but with the architecture flow that humans can actually follow top-to-bottom.
