...
| Cosmology PDF | `python3 scripts/build_cosmology_pdf.py` — ✅ **run canonically at S300 (2026-08-10) on MG's go, and validated end-to-end.** Evidence re-checked S311: the committed `cosmology_formal` `.tex` carries the James-Stein content (12 hits, previously 0) and its abstract reads "Nine" — exactly what `AIW-179` predicted the regeneration would produce; commit `b5efa1c4`. ⚠ Still use `--canonical` deliberately: it overwrites the committed `.tex`+`.pdf` in place, and **a bare invocation with no arguments rebuilds both canonical papers that way** (the Session 106 mechanism). Default output is `tmp/build-cosmology/`. **The former "guarded but never run / do not pass `--canonical`" caveat was STALE — written before MG cleared it and never updated. Corrected S311 2026-08-26.** |
