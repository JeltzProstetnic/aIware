...
- Chapter 13: Human Virtualization
...
During that decade, I finished a degree — after abandoning medicine at the University of Innsbruck, ein Fach das ich ursprünglich wählte um Neurologie zu erforschen — and founded and buried a custom software development startup. I held an "applied research" position in the field of simulation and optimization (the irony is not lost on me), though it was low-maintenance with a generous amount of home office. I taught martial arts. Mainly, I partied.
...
This is the starting point of the Four-Model Theory. It begins with the observation that you have never, in your entire life, directly experienced reality. You have experienced a simulation of reality, generated by your brain, so seamlessly that you have never suspected the difference. And it argues that this observation, taken seriously, dissolves the Hard Problem. blind spot example here, this is a hard sell without a simple argument. also mention here already that the field actually agrees with this preposterous statement.
...
But first, I need to show you the four models. (but first? before what? better Now its time...)
...
NO use the bubble diagram here instead. the old book figures are not to be re-used in this book, they are ALL ugly.

I call them the four models, and they're organized along two axes. (but actually its just two models with sub models, we need to make sure this part is easy to understand!, digital twin analogy here?)
...
but also it felt like being dead!
...
I experienced the time dilation myself. Under salvia, half a second of real time — confirmed by the person watching me — stretched into what felt like fifteen minutes or more. My entire perceptual world rebuilt itself, ran through elaborate sequences (including wings and flying around), and collapsed, all in the time it takes to blink. I described this to an observer who was timing me, and they said I'd been "gone" for less than a second. (the flying feeling came from the air rushing by while i fell back on the bed) The same kind of time dilation I experienced a few years earlier (1998/99) during a near-death event — a mechanism I'll describe in Chapter 14 — but pharmacologically induced and even more extreme. The substrate runs so much content through the simulation so fast that subjective time decouples entirely from clock time. (if i remember correctly, the person who first synthesized salvinorin a and experimented with large doses reported living through a whole lifetime, decades not years, during the about 20 minutes the substance can bind)
...
I know this firsthand. As a teenager, I went through a phase of sleepwalking. One morning I woke to find myself at my desk, with scribbled notes in front of me — written left-handed, which I never do while awake. Looking around the room, I remembered that I'd walked along the walls in a circle trying to go out, not finding the door. The substrate was navigating, motor programs were executing, but the simulation — the "I" — was dark during the phase in which i sat down at my desk and tried to write down what had happened.
...
The Four-Model Theory offers a more specific answer: it depends on which models are running and how degraded they are. (but if you could split the body as well, it would definitely be two persons, and i tend to say it is in any case much closer to two persons than to one, if there is not enough bandwidth between the two half of the brains to synchronize in real time. sharing the same body and most of the same inputs, they tend to cooperate very well most of the time, making it look from the outside more like one person.)
...
The answer is learning (and adaptation, and allowing us to act against learned behavior). Specifically, a kind of learning that unconscious systems simply cannot do.
...
*A computer will repeat this sentence and the following sentence until hell freezes over. Read the previous sentence.* (maybe we can design a prompt that "breaks" llms like this? and prove thereby that llms are not conscious? we will test this before actually incorporating that into the book if successful)
...
Neuromorphic chips — hardware designed to mimic neural dynamics, with analog-like properties, stochastic elements, and massive parallelism — are the most promising direction. They're not conventional digital computers. They're something in between: physical systems designed to have brain-like dynamics at the hardware level. If mind uploading ever works, I suspect the target substrate will look more like a neuromorphic chip than like a server rack running software. (except if we manage to decode the programming language the brain uses to create the virtual systems, then we could read and recompile them digitally, on a symbolic, code level)
...
There is, however, one scenario that changes the calculus entirely.
...
This thought experiment reveals something important about identity. The copy problem exists because copying *interrupts* the simulation. (but the simulation gets interrupted every night, so does this really matter so much? it is less simple. a copy is possible so much is clear. a migration is possible, using the gradual replacement, but *could* also be possible with a radical replacement. from my own amnesic experience from a hard knockout i can only say, existing is more important than remembering, i was lying on the floor worrying what had happened, not worrying who i was before. my opinion: we are all the same basically. being born and reaching consciousness we are not overly irritated at all about that. the problem arises when you put a model fitting a certain biological body into a different body or system, but moving only the virtual models could solve that) There's a moment — however brief — when the original simulation is here and the copy's simulation hasn't started yet. Then there are two simulations. Two streams of experience. Two selves. But gradual replacement avoids this entirely. One simulation, continuous, unbroken. The substrate changes beneath it like replacing planks on a moving ship, but the ship — the simulation, the consciousness, the *you* — never stops sailing.
...
Instant transfer, though — scanning, copying, booting on a new substrate — would hit the simulation with all the changes at once. And that, I think, is where the danger lives. (depending how it is done)
...
And it gets worse. If copies are possible, then *multiple* copies are possible. An army of you. (link to the bobiverse scifi books!) Each one conscious, each one feeling like the original, each one with legitimate claims to your identity, your relationships, your property, your life. The legal and ethical frameworks required to manage this don't exist and can't be improvised. They need to be built with the same care as the technology itself.
...
Actually, I think I do have an answer — or at least the beginning of one. The universe is demonstrably Class 4-capable. Fractals, self-organizing criticality, edge-of-chaos dynamics — they're everywhere, from weather systems to neural tissue to galaxy formation. A Class 4-capable universe is, by definition, capable of universal computation. And a computational substrate of this universe's scale — vast if not infinite in space, time, possibly scale, and probably more dimensions we haven't identified — doesn't merely *allow* self-simulating systems to emerge. It almost guarantees it (if infinite at least in some dimensions). Not as a matter of luck, not as a roll of cosmic dice that happened to come up consciousness. As a structural consequence of what this universe *is*. The remaining mystery is one level deeper: why is there a Class 4-capable universe at all? That, I genuinely don't know but i speculate because nothing cannot exist and is just a platonic idea. But the jump from "Class 4-capable universe" to "conscious beings asking why they're conscious" — that part follows from the architecture.
...
TODO: Uncle Norbert Gruber, one of the first IT professionals in the Rheintal who gave me my first PC. (died some years ago)
...
link to francis crick and robert kochs book about the NCC, which is wrong in its quest but the best treaties of functional neuroanatomy i have read, and i have read quite a few.
