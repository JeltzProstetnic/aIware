...
BACK_COVER_BLURB = r"""Das Ich ist eine Simulation.
...
def build_kindle_cover():
    ...
    ok = build_cover("de", wrap=False)
    if not ok:
        print("  FAILED: could not build base front cover PDF")
        return False

    pdf_path = os.path.join(OUTPUT_DIR, "cover-front-de.pdf")
    ...
    jpg_path = os.path.join(OUTPUT_DIR, "cover-kindle-de.jpg")
    exif = img.getexif()
    ...
    # Copy to desktop
    win_path = f"/mnt/c/Users/Matthias/Desktop/cover-kindle-de.jpg"
    try:
...
def main():
    ...
    parser.add_argument('--edition', choices=['de', 'de-hc', 'all'], default='de',
                        help='Edition: de (paperback), de-hc (hardcover), or all')
    ...
    editions = ['de', 'de-hc'] if args.edition == 'all' else [args.edition]
    results = {}
    for ed in editions:
