...
```mermaid
graph LR
    subgraph IMPL["Substrate (Implicit Models)"]
        IWM["IWM"]
        ISM["ISM"]
    end

    subgraph EXPL["Simulation (Explicit Models)"]
        EWM["EWM<br/>Scene integration"]
        ESM["ESM<br/>Consequence evaluation"]
    end

    IMPL -->|"deploys for evaluation<br/>(primary)"| EXPL
    EXPL -->|"reshapes through learning<br/>(secondary)"| IMPL

    style IMPL fill:#1a1a2e,stroke:#333,color:#aaa
    style EXPL fill:#2d1b69,stroke:#9b59b6,color:#fff
