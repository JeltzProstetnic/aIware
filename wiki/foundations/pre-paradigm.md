...
```mermaid
graph TB
    subgraph Theories["Competing Frameworks"]
        IIT["IIT<br/>(Tononi)"]
        GNW["GNW<br/>(Baars, Dehaene)"]
        HOT["HOT<br/>(Rosenthal)"]
        PP["PP<br/>(Friston, Seth)"]
        AST["AST<br/>(Graziano)"]
        RPT["RPT<br/>(Lamme)"]
    end

    COGITATE["COGITATE<br/>Adversarial Collaboration<br/>(2025)"]
    IIT --> COGITATE
    GNW --> COGITATE
    COGITATE --> Equivocal["Equivocal Results:<br/>Neither fully confirmed"]

    Controversy["IIT Pseudoscience<br/>Controversy<br/>(100+ signatories)"]
    IIT --> Controversy

    Impasse["Pre-Paradigm Impasse:<br/>No theory addresses<br/>all 8 requirements"]
    Equivocal --> Impasse
    Controversy --> Impasse
    HOT --> Impasse
    PP --> Impasse
    ...
    FMT["FMT:<br/>Addresses all 8<br/>requirements"]
    Impasse -.->|"motivates"| FMT

    style Theories fill:#1a1a2e,stroke:#e94560,color:#fff
    style COGITATE fill:#16213e,stroke:#0f3460,color:#fff
    style Equivocal fill:#3b1f2b,stroke:#f472b6,color:#fff
    style Controversy fill:#3b1f2b,stroke:#f472b6,color:#fff
    style Impasse fill:#4a1942,stroke:#e94560,color:#fff
    style FMT fill:#1a3c34,stroke:#34d399,color:#fff
```
