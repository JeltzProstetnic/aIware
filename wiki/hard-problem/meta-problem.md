...
```mermaid
graph TB
    subgraph SUBSTRATE["Substrate Level (Implicit)"]
        ISM["Implicit Self Model<br/>Stores: body schema, habits,<br/>personality, motor programs"]
        IWM["Implicit World Model<br/>Stores: perceptual regularities,<br/>causal models, knowledge"]
    end

    subgraph VIRTUAL["Virtual Level (Explicit)"]
        ESM["Explicit Self Model<br/>The conscious 'I'"]
        EWM["Explicit World Model<br/>The conscious scene"]
    end

    ISM -->|"generates"| ESM
    IWM -->|"generates"| EWM
    ESM -.->|"CANNOT observe"| ISM
    ESM -->|"introspects"| ESM

    BARRIER["Information Barrier<br/>Near-opaque, occasional leaks"]

    style SUBSTRATE fill:#1a1a2e,stroke:#333,color:#aaa
    style VIRTUAL fill:#2d1b69,stroke:#9b59b6,color:#fff
    style ISM fill:#2c3e50,stroke:#34495e,color:#bbb
    style IWM fill:#2c3e50,stroke:#34495e,color:#bbb
    style ESM fill:#8e44ad,stroke:#9b59b6,color:#fff
    style EWM fill:#8e44ad,stroke:#9b59b6,color:#fff
    style BARRIER fill:#c0392b,stroke:#e74c3c,color:#fff
```
