...
```mermaid
graph LR
    subgraph BEFORE["Traditional View"]
        direction TB
        PHY1["Physical Processing<br/>(neurons, synapses)"]
        GAP1["??? Explanatory Gap ???"]
        EXP1["Subjective Experience<br/>(qualia, feelings)"]
        PHY1 ~~~ GAP1 ~~~ EXP1
    end

    subgraph AFTER["Two-Level View (FMT)"]
        direction TB
        VSYS["Virtual System (Level 5)<br/>Experience is CONSTITUTIVE here"]
        SSYS["Substrate (Levels 1-4)<br/>No phenomenal properties"]
        SSYS -->|"generates &<br/>sustains"| VSYS
    end

    BEFORE -->|"dissolves into"| AFTER

    style GAP1 fill:#c0392b,stroke:#e74c3c,color:#fff
    style PHY1 fill:#2c3e50,stroke:#34495e,color:#ccc
    style EXP1 fill:#8e44ad,stroke:#9b59b6,color:#fff
    style VSYS fill:#2d1b69,stroke:#9b59b6,color:#fff
    style SSYS fill:#1a1a2e,stroke:#333,color:#aaa
    style BEFORE fill:#1c1c1c,stroke:#555,color:#ccc
    style AFTER fill:#1c1c1c,stroke:#555,color:#ccc
```
