---
title: "COGITATE and Adversarial Collaborations"
...
```mermaid
graph TB
    subgraph COGITATE["COGITATE Results"]
        direction TB
        IIT_PRED["IIT prediction:<br/>posterior hot zone,<br/>sustained activity"]
        GNW_PRED["GNW prediction:<br/>frontoparietal ignition,<br/>P3b at offset"]
        RESULT["Data:<br/>posterior > frontal,<br/>but neither fully confirmed"]
        IIT_PRED --> RESULT
        GNW_PRED --> RESULT
    end

    subgraph FALLOUT["Fallout"]
        direction TB
        PSEUDO["100+ researchers<br/>call IIT pseudoscience"]
        REBUTTAL["Tononi et al.<br/>rebuttal"]
        IMPASSE["Field impasse<br/>deepened"]
        PSEUDO --> IMPASSE
        REBUTTAL --> IMPASSE
    end

    subgraph FMT_PATH["FMT: Alternative Path"]
        direction TB
        P1["Prediction 1<br/><i>Psychedelics +<br/>anosognosia</i>"]
        P2["Prediction 2<br/><i>Ego dissolution<br/>content control</i>"]
        P3["Prediction 3<br/><i>DID in ESM<br/>networks</i>"]
        P4["Prediction 4<br/><i>Lucid dream<br/>criticality</i>"]
    end

    RESULT --> IMPASSE
    IMPASSE -->|"need new<br/>empirical paths"| FMT_PATH

    style COGITATE fill:#264653,color:#fff,stroke:#1d3557
    style FALLOUT fill:#6a1b2a,color:#fff,stroke:#a4243b
    style FMT_PATH fill:#2d6a4f,color:#fff,stroke:#1b4332
    style IMPASSE fill:#9b2226,color:#fff,stroke:#6a040f
```
