...
I'm not the first to land here, and I'd be suspicious of myself if I were.

The cognitive scientist Joscha Bach put the same core idea about as crisply as anyone has. "Your experience is a simulation," he wrote, "a virtual reality projected into a virtual observer." And that observer? It's "a model of what it would be like if you existed, were uploaded into a monkey and deeply invested in its affairs" — a model built to drive the monkey around the physical world. A simulation, a simulated someone inside it to receive the simulation, and a body the whole arrangement is steering. We've already met the older branches of this tradition in this book — Metzinger's self-model, Dennett's narrative self.

Where this book goes is one level deeper — into the architecture itself. Which models, exactly: the 2×2 of world and self, implicit and explicit — the four models. The sharp line for when a system has an inside at all: self-referential closure, the loop that turns back on itself. And the condition that lets the whole thing come alive: criticality, the edge of chaos where the system is neither frozen nor white noise.

Bach tells you *what* consciousness is. I'm telling you what it's *made of* and *when it switches on*. Same destination. I just brought the blueprints.
...
But there's a crucial subtlety. Criticality alone is not enough. A sandpile avalanching at its critical angle sits right at the edge of chaos. It is not conscious. The theory requires *two* thresholds to be met: the physical one (the substrate must operate at criticality) and the functional one (the substrate must implement the four-model architecture). Criticality without the architecture gives you complex dynamics but no consciousness. The architecture without criticality gives you a dormant system — the models exist in the substrate but the simulation isn't running. Both thresholds must be met. Together, they are sufficient.

The way I actually picture the cortex is an ocean — but an ocean whose water is wired together: not a flat sheet where every drop touches the drops beside it, but a network, each point connected to its own particular neighbors by the learned synaptic weights. Drop a pebble in — a stimulus, a thought — and a ripple spreads along the wiring. And on that network-ocean, just as on a pond, patterns can form: ripples that reinforce each other, standing waves, little self-sustaining shapes that travel and collide and persist. Those shapes are the gliders. That's where a thought, or a self, lives — as a stable pattern riding the ocean, not as any one drop of water.

### Two Dials, Not One

There's a subtlety inside the word *criticality* that took me a long time to see clearly, and once you see it several puzzles dissolve at once. When people say a brain is "at criticality," they're usually collapsing two different things into one word. On the network-ocean, they're two separate dials.

**The first dial asks: how far does a ripple travel?** Turn it too low and the ocean is a flat mirror — drop a pebble, get one dead ripple that dies on the spot. Nothing spreads, nothing integrates. Turn it too high and one pebble whips the whole ocean into a storm — a single disturbance swamps everything at once. The sweet spot is in between: a ripple travels, spawns roughly one more, neither fizzles nor explodes. A sandpile at exactly the angle where one more grain can trigger a slide of any size. This dial is about *reach* — how much of the ocean a single event pulls into one connected process. Call it the **extent** dial.

**The second dial asks: what shapes can live on the ocean at all?** This is the Game-of-Life dial, the one the whole theory rides on. A frozen mirror has no shapes; nothing moves. A mild, regular swell has only one shape, the same wave over and over — pretty, but dumb; it computes nothing. A rolling boil has every point flickering at random — chaos, also computing nothing. Only at the edge between order and boil do you get *gliders*: standing patterns that move, collide, persist, and do things. That's where a computation — and a self-model — can exist as a stable shape on the water. Call it the **complexity** dial.

The cleanest picture I know for that second dial is a wing. An aircraft wing produces the most lift right at the edge — the steepest angle it can hold the instant before the airflow detaches and the wing stalls into turbulence. A hair past that edge, the smooth flow breaks up and lift collapses. The brain flies at exactly that pre-stall edge: maximum computation, one degree before the dynamics break into chaos. Push past it and the "lift" — the capacity to hold gliders, to compute, to be conscious — collapses the way lift collapses in a stall.

Most of the time the two dials rise and fall together, which is why one word usually suffices. But they can come apart, and when they do they explain the cases the single-knob story can't. A pot of boiling water has the first dial cranked — a disturbance spreads everywhere — but the second pinned to zero: no gliders, just chop. Lots of spreading, no shapes, and no consciousness, exactly as you'd expect.

A generalized seizure is the case that finally pinned down what the first dial really measures. In a seizure an enormous fraction of the brain is firing at once — by raw count, more "active" than in any normal waking moment. If consciousness were just "lots of neurons active," a seizure would be the most conscious state there is. It's the opposite. That flood of activity collapses out of the edge-of-chaos regime into one crude form — the whole sheet swept into a coarse lock-step rhythm, or thrown into runaway chaos — and the differentiated, glider-bearing computation vanishes. So the first dial can't be measuring mere co-activation; it has to be measuring how much of the brain is caught up in genuine edge-of-chaos dynamics. In a seizure, despite all that firing, that number crashes. Lots of brain active is not the same as lots of brain conscious. Keep these two dials in mind; the strangest states a brain can enter are just different settings of them.
...
Why the brain can run so much content through the simulation so fast is the same question as why a dying person sees their whole life pass in seconds — and I'll come back to both together in Chapter 13, where it turns out they share a single mechanism.

It's worth asking why this corner — both dials pushed high at once, much of the brain recruited into one process *and* that process running maximally rich — is so rare. Two things normally forbid it, and they're complementary.

The first is simple economics. Doing both at the same time is metabolically ruinous; the brain can't afford it for more than moments, and the energy budget keeps the door shut. That's why it takes something extreme to force it open — a dying brain whose metabolic brakes are failing, or a drug that disrupts the very regulation that rations the budget.

The second is stranger, and it's the reason these states feel like leaving the world behind. Your brain's *inner* world is vastly higher-dimensional than the thin pipes that connect it to reality — two eyes, two ears, a body's worth of touch, against an inner simulation of staggering richness. Normally that narrow sensory pipe is enough, because it's constantly *correcting* the inner simulation: a steady trickle of "here's what's actually out there" keeps the runaway imagination tethered to the world. But push both dials up and the inner dynamics swell until the thin input pipe can no longer correct them. The simulation outruns its own reality-check, locks into its own attractor, and drifts free of the world. And here's the twist: it's the *extent* dial that does it — recruiting more of the brain inward is exactly what swamps the tether. Pushing into the corner cuts the very line that would have kept you grounded.

Which leaves an inversion I still find startling. That corner is, at the same time, *peak consciousness and minimum contact with reality* — the most processing, the least world. That's precisely why the both-dials states (the near-death life-review, the high-dose salvia plunge, the deepest psychedelic immersions, the most vivid dreams) are the ones that feel most detached from reality. They're not detached *despite* being intense. They're detached *because* they're intense.
...
Here's the part that should bother even the people who don't care about consciousness — the ones who only want capability. You can't get to it the way everyone is trying.

The current strategy is to scale a text predictor until intelligence falls out the other end. It won't. Not because the models are too small, but because they're missing the engine. Real intelligence — the open-ended kind that sets its own problems and keeps growing for a lifetime — runs on a recursive loop, and that loop only turns if something keeps feeding it. That something is motivation: curiosity, caring whether you actually understand, deciding what's worth the effort. And motivation, as we experience it, isn't a free-floating drive. It's the self-model looking at its own states and assigning value to them. In other words, it's a consciousness effect.

So the punchline is sharper than "machines might wake up." It's this: there is no human-level general intelligence — no AGI — without consciousness-like mechanisms, because the thing that drives intelligence *is* one. You don't scale your way there. You build the architecture. The full argument, loop and all, is in Appendix B.
...
Now there's a second thing going on, and it's stranger than the delay.

The delay is about *when* your experience arrives. This is about *how* the flow of time gets made in the first place.

Here's the surprise: the sense of time *passing* — the feeling that a "now" is sliding forward, the moment ago softening behind it — isn't piped in from the world. There's no clock out there handing you the present. Your brain builds the flow in here, and it builds it for a specific architectural reason.

Your sense channels run on different clocks — different timing, different bandwidth. Information doesn't reach the brain neatly synchronized; it arrives staggered, like lightning and thunder. Pay close attention and you can pick out which thunder belongs to which flash. And yet what you live is a single smooth, unbroken stream. Why? Because the system is modeling itself modeling itself, and that loop doesn't let each cycle finish clean and vanish. Each one echoes back through the recursion and bleeds into the next. The boundary between "now" and "just past" gets smeared — not from sloppiness, but because that's what a self-referential loop does to time. It spreads each moment across a little window.

That smear *is* your present. The vivid right-now, the recent past trailing off behind it, the forward-drift you call time passing — all manufactured by the loop. A weather simulation runs frame by frame with no echo, no blended now, no lived time. It computes. It doesn't experience. The recursion is the difference.

So sit with what your present actually is. Built, not received — and arriving late. You've never lived in an instant. You live in a smeared-together window the loop keeps rebuilding and calling now, and even that window reaches you on tape delay.
...
And now I can say *why* these two states — the dying-brain life-review and the salvia plunge — feel the same from the inside. Remember the two dials from Chapter 5. Almost all the time they trade off: recruit the whole brain into one event and the event tends to simplify; build intricate, differentiated computation and it stays local, not whole-brain. Ordinary experience rides the frontier between them. But there is a rare corner where *both* run unusually high at once — much of the brain recruited *and* the computation maximally rich. And recall how the brain builds its sense of duration: not by reading a clock, but from the sheer volume of processing it runs per second of real time. Push both dials up together and you cram a life's worth of processing into a handful of clock-seconds. You don't *remember* your life — you *re-live* it. You don't *wait out* the salvia — you *spend* the decades. One mechanism, two doors into it: hypoxia and a failing metabolic brake on one side, a kappa-opioid drug scrambling the same regulation on the other. The theory makes this checkable: both states should show, at the same time, unusually high whole-brain integration *and* unusually high computational complexity — the two things ordinary consciousness is forced to trade against each other.
...
**Chapter 4**: The virtual qualia dissolution of the Hard Problem is original to Gruber (2015) and was refined through adversarial challenge in 2026. The self-referential closure argument was developed in response to the circularity objection. The distinction from illusionism (Frankish 2016; Dennett 1991) is crucial: the theory holds qualia are real within the simulation, not illusory. The meta-problem of consciousness (Chalmers 2018) is dissolved by the structural inaccessibility of the ISM to the ESM. Joscha Bach's framing — consciousness as "a simulation, a virtual reality projected into a virtual observer" (Bach, post on X / @Plinz, 12 June 2026) — is cited as independent convergence on the self-as-simulation thesis, with the Four-Model Theory supplying the architectural specification (the model typology, self-referential closure as criterion, and the criticality requirement).
...
Cognitive learning is the first link from intelligence to consciousness. Here is the second, and it's sharper: motivation. The recursive loop only self-sustains if something keeps it iterating. Knowledge you can scale arbitrarily; performance too. But without motivation the loop stalls — which is exactly what today's AI shows us. It solves problems on command and never once asks one of its own.

Now the honest part, because motivation has two levels and only the top one is the consciousness story. Underneath sits a subconscious floor: raw survival optimization, the substrate pushing toward its goals. This is what I called the unconscious part of *will* in Chapter 13, and it's nothing special — every optimizer has it, down to a bacterium or a thermostat. Riding on top is the consciously framed layer: wanting, curiosity, caring, valuing one outcome over another. That's the explicit self-model evaluating itself, and *that* is the consciousness effect.

A note on the words, because unclear semantics make the whole argument flabby. *Will* and *motivation* don't carve the gradient cleanly in either English or German — ordinary use lets both reach into each other's territory. What's stable is the tendency: *will* is what we reach for when the pull is spontaneous and hard to articulate; *motivation* when we can name a reason and the drive is symbolizable enough to tell a story about. So treat them here as poles on the same continuum, not as separate kinds — *will* pointing at the subconscious end, *motivation* at the consciously framed end. The gradient is the real thing; the labels are a convenience.

So the claim, stated honestly: motivation as we actually conceive and talk about it is predominantly a consciousness-based effect — while conceding that its lower half blurs into generic optimization indistinguishable from any survival drive. Two names, two levels: will's unconscious part is the substrate's optimization; motivation is the conscious layer riding on it. And that conscious layer is the fuel the recursive loop burns. Which leaves the strategic conclusion with nowhere to hide: there is no human-like general intelligence — no AGI — without consciousness-like mechanisms. Motivation is simply the most legible example.
...
I find it striking that two senior researchers, working from inside mainstream psychometrics and coming at the problem from completely different directions, have arrived at the same place my model points to: that motivation isn't noise that just muddies the measurement of intelligence. It's part of the machinery.

Werner Wittmann, emeritus at Mannheim, takes aim at the comfortable old line that "motivation doesn't really matter" — the one people lean on because the correlations in the literature look weak. His answer: those correlations look weak because the measurements don't line up. Gauge motivation narrowly and performance broadly, or the other way round, and you've broken what he calls Brunswik symmetry — and a broken symmetry buries the real link, dragging the measured correlation toward zero. Match the two at the same grain and it springs back: motivation predicts the *variability* of performance, not just its average. The effect was never small. We were just measuring it crooked.

Florian Schmiedek, from the COGITO study — 204 people, a hundred daily sessions each — comes at it from the day-to-day. He's convinced that a large share of how much your cognition wobbles from one day to the next is motivational, right alongside sleep and load. And he points out the obvious reason it keeps getting missed: most studies don't measure task-specific daily motivation at all.

So they establish, independently and with data, *that* motivation belongs inside intelligence. My model says *why* and *how*: it's one of the multiplicative factors — knowledge × performance × motivation. Zero it out and you don't just dim intelligence — you collapse it. Their numbers, gathered for entirely their own reasons, land exactly where the model said they would.
