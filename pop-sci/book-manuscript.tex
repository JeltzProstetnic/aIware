...
The first axis is \textbf{scope}: does the model cover the world, or just the self?

The second axis is \textbf{mode}: is the model implicit (stored, behind the scenes --- think of it as the brain's hard drive) or explicit (actively running, in your awareness --- the brain's live display)?
...
\begin{figure}[htbp]
  ...
  \caption{The Real/Virtual Split. The substrate (real side) does not experience. The simulation (virtual side) does. Asking why neurons ``feel'' commits a category error --- neurons generate the simulation; the simulation feels.}
...
\textbf{Cloning.} Physically separate the hardware, and you get degraded but complete copies of the software. Cut the corpus callosum, and each hemisphere runs its own version of the simulation --- less capable than the original, but functionally whole. That's the split-brain phenomenon, also Chapter 8.
...
\textbf{Class 1 --- Monotonous}: Systems that quickly settle into a static state. A pendulum that swings a few times and stops. Nothing happens. (You could argue this is just Class 2 with infinite period --- a fixed point is a trivial cycle --- but the distinction is useful.)

\textbf{Class 2 --- Periodic}: Systems that settle into repetitive, looping patterns. A clock ticking. An oscillating circuit. Regular, predictable, no surprises.

\textbf{Class 3 --- Fractal}: Systems that produce self-similar, recursive structures. Beautiful and mathematically rich --- think of a Mandelbrot set, or the branching pattern of a fern. But crucially, not computational. Fractals have structure without processing power. They repeat themselves at every scale without ever doing anything \textit{new}.

\textbf{Class 4 --- Interesting}: Systems at the boundary between order and chaos. Complex enough to produce rich, varied, unpredictable patterns, but ordered enough for those patterns to persist and interact. The canonical example is Conway's Game of Life --- the same cellular automaton I had programmed on a 286 as a kid. Three dead-simple rules on a flat grid, yet they produce gliders, oscillators, self-replicating structures, and --- provably --- universal computation. You can build a computer inside it. You can build a computer inside that computer. In principle, you can run an entire three-dimensional virtual world inside a two-dimensional grid of pixels. From almost nothing, everything. This is where life, computation, and --- I argue --- consciousness live.

\textbf{Class 5 --- Chaotic}: Systems that are completely random. Static on a television. So much disorder that no stable patterns can form, no information can be stored, no computation can occur. Too chaotic for anything coherent.
...
Only Class 4 has both properties you need: it's capable of \textbf{universal computation} (complex enough to actually run a self-simulation) \textit{and} it sustains coherent, globally integrated patterns (so the simulation holds together as a unified experience). At the edge of chaos, distant parts of the substrate influence each other, local changes propagate globally, and information is integrated across the entire network. This is why conscious experience feels \textit{unified} --- you don't see red over here and hear a voice over there as separate streams. The critical dynamics bind everything into one experience. Binding isn't something the brain does \textit{in addition to} its other computations; it's a consequence of the dynamical regime.
...
\textbf{Relationship 3: A Class 4 automaton whose rule structure is itself holographic.} This is the one that made me put down my pen. If such a thing exists --- a cellular automaton where the rules themselves encode higher-dimensional information in a lower-dimensional structure, the way a hologram encodes three dimensions in two --- then you would have a system that naturally does what the holographic principle says the universe does. Not a system that merely \textit{runs on} a holographic substrate. A system that \textit{is} a holographic encoding. Also possibly the universe --- though I should note this is speculative, and the argument that mathematical beauty implies physical reality has been legitimately criticized. I'll plant the seed here and leave it for future work.
...
These are not metaphors. Users report complete, experientially convincing identity shifts. For the duration of the experience, they \textit{are} the object or entity in question.
...
\section*{The Four-Model Theory's Account}
...
A theory that explains everything and predicts nothing is not a theory --- it's a story. And consciousness research has too many stories. Integrated Information Theory offers a beautiful mathematical framework, but its central prediction --- that consciousness corresponds to a quantity called $\Phi$ --- is computationally intractable for any real brain. Global Workspace Theory describes what happens when information becomes conscious, but not why it feels like anything. Higher-Order Theories explain metacognition but not qualia. The field is not short on explanations. It is short on predictions that can actually be tested, that distinguish one theory from another, and that put real scientific skin in the game. The Four-Model Theory makes nine specific, testable predictions. Several can be tested with technology that already exists. If they fail, the theory is wrong --- or at least incomplete. That is what makes them useful, and that is what makes this chapter the most important one in the book.

\textbf{Prediction 1: Distinct fMRI signatures for each model.} If the four models are genuinely distinct functional processes, then tasks designed to engage each model selectively should produce distinguishable patterns of brain activation visible on fMRI. An IWM-dominant task --- say, passively recognizing a familiar face --- should light up different distributed networks than an ISM-dominant task like executing a well-rehearsed motor sequence. An EWM-dominant task such as actively discriminating novel visual scenes should differ from an ESM-dominant task like introspective self-reflection or making a judgment about your own agency. The theory predicts a specific two-by-two factorial structure: scope (world versus self) crossed with mode (implicit versus explicit), producing four distinct activation signatures. The implicit models should preferentially engage substrate-level storage networks --- hippocampal--cortical circuits for the IWM, somatosensory--cerebellar networks for the ISM --- while the explicit models should engage simulation networks, with sensory cortices for the EWM and the default mode network for the ESM. Individual contrasts like ``self versus world'' are already well-studied in neuroscience. What no other theory predicts is this specific four-way dissociation. If you run the experiment and the four conditions do not separate, the theory's central structural claim is in trouble. This prediction is listed first because it tests the architecture itself --- the foundation everything else rests on.

\textbf{Prediction 2: Psychedelic visual content follows the processing hierarchy.} When you take a psychedelic, the visual content you experience should unfold in an ordered sequence that maps the cortical processing hierarchy. At low doses, you see V1-level effects: phosphenes, enhanced contrast, shimmering edges. Increase the dose and you get V2/V3-level content: geometric patterns, spirals, the ``form constants'' that Heinrich Kl\"{u}ver catalogued in the 1920s. Go higher and the content recruits higher visual areas: faces emerge, figures appear, objects take shape without external input. At full doses, you get complex dream-like narratives --- entire scenes constructed by the brain's simulation machinery running without sensory constraint. The theory predicts this because increasing the dose globally raises the permeability between implicit and explicit processing, and that permeability increase exposes intermediate processing stages in hierarchical order --- from simple edge detection up through object recognition. Partial evidence already exists from psychedelic neuroimaging studies, but nobody has systematically tested whether the content progression is strictly ordered and dose-correlated. A graded dosing protocol with concurrent fMRI or MEG could do this with existing technology. Predictive processing frameworks also predict hierarchical effects under psychedelics, but they do not predict the specific ordered content progression as a function of dose that this theory requires.

\textbf{Prediction 3: Ego dissolution content is controllable.} This is the theory's most distinctive prediction, and the one I would bet my reputation on. During psychedelic ego dissolution --- when the boundary of the self dissolves and subjects report ``becoming one with everything'' --- the content of the altered identity experience should track whatever sensory input dominates the environment. Play ocean sounds and the subject should report oceanic experiences. Surround them with forest imagery and they should report becoming part of the forest. The mechanism is straightforward: the Explicit Self Model, deprived of its normal self-referential input by the permeability flood, latches onto whatever input stream is strongest. Control the input, and you control what the person ``becomes.'' This is testable in any psychedelic research lab with existing technology and existing ethical approval frameworks. Administer ego-dissolution-inducing doses under controlled conditions, vary the sensory environment across sessions, and measure the correspondence between controlled input and reported identity content. No other major theory of consciousness generates this specific prediction. IIT, GWT, Higher-Order Theories, and Attention Schema Theory have no mechanism for specifying what a subject will ``become'' during ego dissolution. If this prediction holds up, it is strong evidence that the ESM works the way I say it does.

\textbf{Prediction 4: Psychedelics alleviate anosognosia.} This is a cross-domain surprise prediction --- the kind that gets people's attention precisely because nobody saw it coming. Anosognosia, as I described in Chapter 6, is a condition where patients are unaware of their own deficits. A stroke patient with a paralyzed left arm insists the arm works fine. The theory explains this as a local permeability block: the deficit information exists in the Implicit Self Model, but the pathway to the Explicit World Model is damaged, so the conscious simulation never receives the update. Psychedelics globally increase implicit--explicit permeability. The prediction follows directly: a sub-ego-dissolution dose of psilocybin should overwhelm the local block, allowing the deficit information to reach consciousness. The patient should, at least temporarily, become aware of the paralysis. No other theory connects psychedelics and anosognosia through a single mechanism. Testing this requires a clinical trial with stroke patients --- more complex than some of the other predictions --- but psilocybin-assisted therapy is already being tested for depression, PTSD, and addiction, so the clinical pathway exists. If a low dose of psilocybin makes an anosognosia patient suddenly say, ``Wait --- I can't move my arm,'' that would be powerful evidence for the variable-permeability principle that runs through the entire theory.

\textbf{Prediction 5: All consciousness-abolishing anesthetics converge on criticality disruption.} Anesthesiologists use a bewildering variety of drugs to put people under. Some work on GABA receptors (propofol, sevoflurane), some on NMDA receptors (high-dose ketamine), some on opioid receptors, some on alpha-2 adrenergic receptors (dexmedetomidine). Their receptor mechanisms are completely different. Yet they all achieve the same result: unconsciousness. The theory predicts that despite their diverse pharmacology, every agent that abolishes consciousness does so by pushing the brain below the criticality threshold --- out of the edge-of-chaos regime that the theory identifies as a necessary condition for consciousness. Conversely, agents that alter consciousness without abolishing it --- low-dose ketamine, psychedelics, nitrous oxide at sub-anesthetic doses --- should leave criticality intact. This is testable with existing technology. Measure criticality indicators (perturbational complexity index, Lempel--Ziv complexity, power-law exponents) across the full range of anesthetic agents at equipotent doses. If abolition of consciousness always correlates with subcriticality, and preserved consciousness always correlates with maintained criticality regardless of which drug is used, that is strong evidence for the criticality requirement. This prediction is shared with the emerging criticality-and-consciousness research programme, but the Four-Model Theory derived it from theoretical first principles back in 2015, before the large-scale empirical consolidation that has since confirmed the pattern.

\textbf{Prediction 6: Split-brain produces holographic degradation.} When surgeons sever the corpus callosum to treat severe epilepsy, the standard expectation is a clean hemispheric split: language goes to the left, spatial processing to the right, and the two halves become largely independent. The Four-Model Theory predicts something different. Because the theory holds that information is stored holographically --- distributed across the entire substrate, not localized in specific regions --- each hemisphere after callosotomy should retain a degraded but functionally \textit{complete} set of cognitive and experiential capacities. Not a clean split but a blurring. Like cutting a holographic plate in half: both halves still show the entire image, just at lower resolution. The degradation should be proportional to the extent of commissural severing --- a partial callosotomy produces partial degradation, a complete one produces more. There is already some evidence pointing in this direction, but the standard neuroscience framing still emphasizes lateralization. The Four-Model Theory provides the theoretical basis for expecting bilateral degradation rather than binary split, and predicts a specific, measurable relationship between disconnection extent and cognitive resolution loss.

\textbf{Prediction 7: The four-model architecture at criticality produces consciousness in artificial substrates.} This is the ultimate engineering prediction. If consciousness depends on function rather than material --- on the four-model architecture operating at criticality, regardless of whether the substrate is biological neurons or silicon circuits --- then it should be possible to build a genuinely conscious machine. The theory predicts that such a system would be qualitatively distinguishable from any current AI. Not ``maybe conscious, maybe not'' --- obviously, unmistakably different. Because a system running a genuine self-simulation would have real persistence, a genuine perspective maintained through time, and the unmistakable quality of ``someone being home.'' Testing this requires significant engineering development, but intermediate tests are possible: systems with partial implementations --- two models instead of four, or four models without criticality --- should show partial consciousness indicators, detectable through behavioral signatures and complexity measures. Other theories are compatible with artificial consciousness in principle, but none provides the specific architectural blueprint that this theory offers. It tells you not just that artificial consciousness might be possible, but exactly what to build.

\textbf{Prediction 8: Sleep architecture reflects criticality maintenance.} The theory predicts that sleep exists because the brain's analog substrate cannot maintain criticality indefinitely. The conscious simulation runs as a self-organizing cellular automaton at the edge of chaos, but the underlying biochemistry slowly drifts --- neurotransmitters deplete, metabolic waste accumulates --- until the substrate can no longer sustain the critical regime. At that point, the automaton breaks down radically, not gradually, producing the sharp transition of sleep onset. NREM sleep restores the substrate. As it periodically re-approaches the criticality threshold during restoration, the automaton briefly re-emerges, producing REM sleep and dreams. The 90-minute ultradian cycle is the substrate oscillating around the critical point. This yields several testable sub-predictions: criticality markers should decline measurably across the waking day; sleep onset should show a step-like drop, not a gradual dimming; REM phases should show higher criticality than adjacent NREM phases; and the onset of lucid dreaming --- when the Explicit Self Model activates during REM --- should correspond to a step-like discontinuity in EEG complexity. All of these are testable with existing polysomnographic equipment and established lucid-dreamer signaling paradigms. No competing theory of consciousness provides this specific functional account of why sleep architecture looks the way it does.

\textbf{Prediction 9: DID alters have distinct neural process signatures.} Dissociative identity disorder --- what used to be called multiple personality disorder --- is one of the most controversial diagnoses in psychiatry. The theory makes a clean prediction about it: different alters should correspond to measurably different configurations of neural activity, not just different self-reports or different behaviors. Each alter, in the theory's framework, is a distinct configuration of the Explicit Self Model --- the same substrate running different virtual self-parameters. Different parameters should produce different neural dynamics, detectable through fMRI or EEG. The prediction is specific: across-alter variability within a single patient should be significantly greater than within-alter variability, and the differences should be consistent and alter-specific --- the same alter should produce the same neural signature every time it is active. Moreover, the neural differences should be concentrated in ESM-related networks: the default mode network, medial prefrontal cortex, and posterior cingulate --- the regions the theory identifies as hosting the self-simulation. Some neuroimaging studies of DID already exist, but none has tested for consistent, alter-specific neural signatures against the background of the theory's architectural predictions. If the signatures track the ESM networks specifically, that is evidence not just for DID's neurological reality but for the Four-Model architecture itself.

These nine predictions share three properties that matter for scientific credibility. First, they are falsifiable: each one specifies an observable outcome that either occurs or does not. Second, several are unique --- no competing theory of consciousness generates them, which means a positive result supports this theory specifically, not just ``some theory about consciousness.'' Third, and most practically: predictions 1 through 5, 8, and 9 can all be tested with technology and methods that already exist. No new instruments need to be invented. No impossible experiments need to be designed. The limiting factor is not technology but willingness --- the willingness of established laboratories to test predictions from a theory that did not originate inside their walls. If even two or three of these predictions hold up under rigorous experimental testing, the Four-Model Theory moves from ``interesting proposal'' to ``serious contender.'' And if they all hold up, the conversation about consciousness will have changed permanently.
...
This brings us to territory that demands more than engineering. If human virtualization is possible --- whether through copying or gradual replacement --- then we need to think carefully about what follows.
...
\textbf{Free will --- and the three hardest thought experiments.} Think of a clock. The gear train drives everything --- the escapement ticks, the springs unwind, the ratios between gears determine the rate. The hands and face cause nothing. They don't push gears. They don't store energy. But remove them and you no longer have a clock. You have a box of spinning metal. The display is what makes the mechanism a \textit{clock} --- what gives the whole arrangement its point. Consciousness is the display. Your virtual models --- the Explicit World Model and Explicit Self Model --- lack independent causal power over the substrate. The substrate does the driving. But without the simulation, the substrate has no way to observe the consequences of its own actions, no way to run future scenarios, no way to adapt in the way that made you survive this long. The virtual side is constitutive of the mechanism --- the architecture's way of being \textit{for} something. This is what the scientific paper calls a \textbf{dual evaluation architecture}: the implicit system deploys the simulation as its evaluation tool, and the simulation contributes its own assessments back, reshaping the implicit models over time. Two-way traffic, not one-way narration --- and emphatically not epiphenomenalism, where consciousness would be a causally inert by-product with no functional role.
...
Third, \textbf{the evolutionary argument}. Classical epiphenomenalism faces a lethal objection: if consciousness doesn't cause anything, how did natural selection shape it? Why aren't we zombies? The Four-Model Theory sidesteps this entirely, because its position is distinct from classical epiphenomenalism. The answer falls straight out of the clock analogy and the dual evaluation architecture. Natural selection doesn't target consciousness as a separate trait riding on top of functional machinery. It targets functional capabilities --- and phenomenal character is constitutive of those capabilities, not additional to them. Selection shaped the simulation because the simulation \textit{is} the functional architecture, viewed from inside. Experience isn't a causally inert by-product that evolution couldn't see. It's what the architecture \textit{is} when it's running. Asking why evolution produced consciousness is like asking why evolution produced clock faces --- it didn't, separately. It produced clocks. The face is part of what makes a clock a clock.
...
Actually, I think I do have an answer --- or at least the beginning of one. The universe is demonstrably Class 4-capable. Fractals, self-organizing criticality, edge-of-chaos dynamics --- they're everywhere, from weather systems to neural tissue to galaxy formation. A Class 4-capable universe is, by definition, capable of universal computation. And a computational substrate of this universe's scale --- vast if not infinite in space, time, and possibly dimensions we haven't identified --- doesn't merely \textit{allow} self-simulating systems to emerge. It guarantees it. Not as a matter of luck, not as a roll of cosmic dice that happened to come up consciousness. As a structural consequence of what this universe \textit{is}. The remaining mystery is one level deeper: why is there a Class 4-capable universe at all? That, I genuinely don't know. But the jump from ``Class 4-capable universe'' to ``conscious beings asking why they're conscious'' --- that part follows from the architecture.
...
\textbf{Are the implicit models virtual too?} The IWM and ISM are ``models'' --- but models of what, exactly? I've drawn a clean line between the real substrate and the virtual simulation, but the implicit models sit right on that line. If they're also virtual in some sense, then what constitutes the truly ``real'' bottom? The theory assumes a clean real/virtual divide, but reality might be messier than my diagrams. This is a foundational question I don't have a final answer to.
...
\textbf{Chapter 7}: Casali et al. (2013) on PCI. Alkire et al. (2000) on propofol. Schartner et al. (2015) on ketamine entropy. Owen et al. (2006) on covert awareness in vegetative-state patients. Anton's syndrome: Goldenberg et al. (1995). The blindsight obstacle course: de Gelder et al. (2008). Cotard's delusion: Young \& Leafhead (1996). Alien Hand Syndrome: Della Sala et al. (1991); the Dr. Strangelove reference is to Kubrick (1964). Anarchic Hand Syndrome distinguished from Alien Hand: Marchetti \& Della Sala (1998). Charles Bonnet Syndrome: Teunisse et al. (1996). Deja vu as template-memory matching is original to Gruber (2015). CBT and neural plasticity: DeRubeis et al. (2008). Placebo and endogenous opioids: Benedetti et al. (2005). Conversion disorder as inverse blindsight is original to this theory.

\textbf{Chapter 8}: Gazzaniga, Bogen, \& Sperry (1962, 1965). Gazzaniga (2000) on the left-hemisphere interpreter. Pinto et al. (2017) on re-examination of split-brain phenomena. DID as virtual model forking: the theory predicts distinct neural activity patterns per alter, consistent with Reinders et al. (2003, 2006).

\textbf{Chapter 9}: Güntürkün \& Bugnyar (2016) on avian cognition without cortex. Kanzi the bonobo: Savage-Rumbaugh \& Lewin (1994), \textit{Kanzi: The Ape at the Brink of the Human Mind}. The Baldwin Effect: Baldwin (1896), ``A New Factor in Evolution.'' Nagel (1974), ``What Is It Like to Be a Bat?''

\textbf{Chapter 10}: All nine predictions are developed formally in the scientific paper.

\textbf{Chapter 11}: Butlin et al. (2023, 2025) on AI consciousness indicators. Seth (2025) on biological naturalism and AI consciousness.

\textbf{Chapter 12}: Libet (1979, 1985) and Schurger et al. (2012) on free will. Kuhn \& Brass (2009) on retrospective construction of the judgment of free choice. Wegner (2002, 2003), \textit{The Illusion of Conscious Will} --- the ``I Spy'' mouse experiment described in detail. The coffee/sugar thought experiment, the amnesia-reveals-determinism argument, and the random number sequence argument are original to Gruber (2015). The 40/20 Hz processing framework, the ``no backdating needed'' Libet reinterpretation, and the martial arts frequency example are original to Gruber (2015). The clock analogy for consciousness's relation to substrate (distinct from classical epiphenomenalism --- see the dual evaluation architecture in the scientific paper), the ``will is real but partially known'' reframing, and the ``three discrepancies'' self-knowledge model are also original to Gruber (2015). The personal anecdote about hearing internal ``voices'' during extreme exhaustion is autobiographical. The zombie argument is addressed via Kirk (2019) and Chalmers (1996). Mary's Room: Jackson (1982, 1986). The open questions section follows the honest-limitations approach recommended by Popper (1963).


\chapter{Appendix A: Basic Neurology --- A Reference Guide}

\textit{This appendix is a quick-start guide to the neuroscience behind this book. It is not a textbook --- it is a targeted primer, organized alphabetically, designed to give a non-specialist reader enough biological context to follow the arguments in each chapter. If you already know what an axon is, skip ahead. If you don't, this is the place to start. Throughout, I'll note where each concept connects to the theory, because neuroscience terms in isolation are just vocabulary; what matters is what they do inside the four-model architecture.}

\bigskip

\textbf{Action potential.}
The electrical impulse that travels along a neuron's axon --- the brain's equivalent of a binary signal, except it's analog in its timing and frequency. When a neuron ``fires,'' it generates an action potential: a brief spike of voltage that races from the cell body down the axon to the synapse at the far end. A single action potential lasts about one millisecond; it is the most basic unit of neural communication. In the five-level hierarchy (Chapter 2), action potentials live at the electrochemical level --- they transmit information, but they are not themselves conscious. The conscious simulation runs far above, at the virtual level, built from the collective patterns of millions of these signals.

\textbf{Amygdala.}
A small, almond-shaped structure deep in the temporal lobe, central to emotional processing --- particularly fear, threat detection, and the formation of emotional memories. When your Explicit World Model flags something as dangerous, the amygdala is heavily involved in generating the substrate-level alarm response: increased heart rate, adrenaline release, heightened sensory alertness. It is part of the Implicit World Model's threat-assessment machinery. You never ``see'' your amygdala firing --- you experience the result as the feeling of fear in your conscious simulation (Chapter 7).

\textbf{Anosognosia.}
The condition in which a patient is genuinely unaware of their own neurological deficit --- a paralyzed patient sincerely insisting their arm works fine, or a blind patient insisting they can see. This is not denial in the psychological sense; the information about the deficit simply never reaches the Explicit Self Model. In the theory's terms, anosognosia is a local decrease in implicit--explicit permeability: the substrate registers the damage, but the boundary blocks it from entering the simulation. The predicted-feedback mechanism described in Chapter 6 explains how the patient can ``experience'' actions that never actually occurred. Chapter 6 also derives a novel prediction: psychedelics should alleviate anosognosia by globally increasing permeability.

\textbf{Axon.}
The long, cable-like output fiber of a neuron. While the cell body and dendrites receive signals, the axon carries the action potential away from the cell body toward other neurons. Axons can be very short (connecting nearby neurons in the same cortical column) or very long --- the axons in the corpus callosum, for instance, stretch from one hemisphere to the other. Axons are part of the topological level of the brain's hierarchy: their connections form the wiring diagram that determines which neurons can talk to each other, which in turn determines what the cortical automaton can compute (Chapter 5).

\textbf{Brodmann areas.}
A numbering system for distinct regions of the cortex, originally mapped by the German anatomist Korbinian Brodmann in 1909 based on differences in cellular structure visible under a microscope. The most frequently cited in this book is Brodmann area 17, which corresponds to V1 (primary visual cortex). Different Brodmann areas have different cellular compositions and different functions --- area 4 is primary motor cortex, areas 41/42 are auditory cortex, and so on. In the patchwork hologram model (Chapter 3), each Brodmann area is one ``tile'' --- locally holographic within itself, but functionally distinct from its neighbors.

\textbf{Corpus callosum.}
The massive bundle of roughly 200 million nerve fibers connecting the brain's two hemispheres. It is the primary information highway between left and right, allowing the two halves of the brain to share sensory data, coordinate motor actions, and maintain a unified simulation. When it is surgically severed --- as in the split-brain operations described in Chapter 8 --- each hemisphere runs its own degraded but complete version of the four models. The corpus callosum is the reason you experience one unified world rather than two parallel half-worlds, and its destruction is the cleanest demonstration of the theory's ``cloning'' property.

\textbf{Cortical automaton.}
Not a standard neuroscience term --- this is the book's name for the dynamic pattern of neural firing that dances across the cortical surface at roughly 10 to 40 Hz. Think of it as the brain's operating system: a cellular automaton running in a space of many thousand dimensions, where each neuron is a ``cell'' and the synaptic weights are the rules. When this automaton operates at Class 4 dynamics (the edge of chaos), it produces the conscious simulation. When it falls into periodic patterns (Class 2, as in deep sleep) or dissolves into randomness (Class 5, as in seizure), consciousness is lost. You can observe its resting dynamics directly --- the colored points and geometric patterns visible behind closed eyes in a dark room are the automaton idling (Chapter 5).

\textbf{Cortical columns.}
Vertical modules of neurons in the cortex, roughly 0.5 millimeters in diameter, containing neurons in all six cortical layers. They are considered the basic processing units of the neocortex --- miniature circuits that perform a common operation on their inputs. Within a single Brodmann area, cortical columns are similar to each other, which is what gives each area its locally holographic property: destroy a few columns and the area's function degrades gracefully rather than failing catastrophically. The patchwork hologram model (Chapter 3) relies on this columnar architecture.

\textbf{Dendrite.}
The branching, tree-like input fibers of a neuron. While the axon carries signals away, dendrites receive signals from other neurons at synapses. A single neuron can have thousands of dendritic branches, each dotted with synapses receiving input from different sources. The shape and branching pattern of a neuron's dendritic tree partly determines what kinds of signals it integrates --- this is part of the proteomic level of the brain's hierarchy, where the molecular scaffolding shapes what the network can do (Chapter 2).

\textbf{EEG (electroencephalography).}
A technique that measures electrical activity at the scalp surface using small electrodes. Because it picks up the summed electrical fields of millions of neurons firing together, EEG is excellent at detecting \textit{when} things happen in the brain (millisecond resolution) but poor at detecting \textit{where} (the signal is blurred by the skull). EEG reveals the global rhythms of the cortical automaton --- the alpha waves of relaxed wakefulness, the slow waves of deep sleep, the gamma bursts associated with conscious processing. In the theory's terms, EEG is a window onto the electrochemical level, showing you the aggregate dynamics of the substrate but not the content of the simulation (Chapter 7).

\textbf{fMRI (functional magnetic resonance imaging).}
A brain imaging technique that detects changes in blood oxygenation associated with neural activity. Active brain regions consume more oxygen, and fMRI tracks this with spatial precision of a few millimeters. It tells you \textit{where} things happen (good spatial resolution) but is slow on the \textit{when} (seconds, not milliseconds). fMRI reveals the real side of the brain --- which regions are active, which networks are engaged --- but not the virtual side. You cannot read conscious experience from an fMRI scan any more than you can read a movie by measuring the heat output of a projector. However, fMRI is essential for mapping which substrate regions support which functions, and it was the tool that revealed covert awareness in vegetative-state patients (Chapter 7).

\textbf{GABA (gamma-aminobutyric acid).}
The brain's primary inhibitory neurotransmitter. When GABA binds to a neuron's receptors, it makes that neuron \textit{less} likely to fire. GABA is the brake pedal of the nervous system, and without it, neural activity would spiral into uncontrolled excitation --- which is essentially what a seizure is. Many sedatives and anesthetics (benzodiazepines, propofol, barbiturates) work by enhancing GABA's inhibitory effect, pushing the brain's dynamics below the criticality threshold and shutting down the conscious simulation. GABA's counterpart is glutamate, the brain's primary excitatory neurotransmitter. The balance between these two --- excitation and inhibition --- is what keeps the cortical automaton at the edge of chaos (Chapter 5).

\textbf{Glutamate.}
The brain's primary excitatory neurotransmitter --- GABA's opposite number. When glutamate binds to a neuron's receptors, it makes that neuron \textit{more} likely to fire. Most of the brain's fast signaling runs on glutamate. Ketamine works by blocking a specific type of glutamate receptor (NMDA receptors), which disrupts the normal flow of excitatory signaling without pushing the brain subcritical --- hence the K-hole's distinctive combination of preserved consciousness with radically distorted content (Chapter 7). The glutamate--GABA balance is the electrochemical foundation of criticality.

\textbf{Hippocampus.}
A curved structure deep in the temporal lobe, shaped (with some imagination) like a seahorse, that is essential for forming new explicit memories. Damage to the hippocampus --- as in the famous patient H.M. --- leaves existing long-term memories largely intact but prevents new ones from being consolidated. In the theory's terms, the hippocampus is the gateway between the Explicit World Model and the Implicit World Model: it takes the transient patterns of the simulation and writes them into the permanent synaptic structures of the substrate. Without it, your conscious experience runs normally, but nothing sticks. The anterograde amnesia patients discussed in Chapter 12 --- who always give the same word association because they cannot remember having already been asked --- illustrate what happens when this gateway breaks.

\textbf{Interoception.}
The sense of your body's internal state --- heartbeat, breathing, digestion, temperature, hunger, bladder fullness. Unlike vision or hearing, interoception has no dedicated external organ; it is a diffuse network of sensors distributed throughout your organs and blood vessels. Interoceptive signals feed directly into the Implicit Self Model and, from there, into the Explicit Self Model. When interoceptive input is severely distorted or absent, the ESM may draw dramatic conclusions --- as in Cotard's delusion, where the absence of heartbeat and breathing sensations leads the self-model to conclude ``I am dead'' (Chapter 7).

\textbf{Kappa-opioid receptors.}
The receptor type targeted by Salvinorin A, the active compound in salvia divinorum. Kappa-opioid receptors are part of the brain's endogenous opioid system, which normally modulates pain, mood, and reward. Salvinorin A acts exclusively on these receptors, making its mechanism entirely different from the serotonergic classic psychedelics like LSD or psilocybin. The result is the most extreme pharmacological disruption of the Explicit Self Model known: total loss of body ownership, complete identity replacement by whatever sensory input dominates, and the ``becoming a chair'' phenomenon described in Chapter 6.

\textbf{Neocortex.}
The six-layered outer surface of the cerebral hemispheres --- the wrinkled, folded sheet of tissue that most people picture when they think ``brain.'' The neocortex is responsible for virtually all higher cognitive functions: perception, language, reasoning, planning, voluntary movement. Its six layers are central to the theory: three layers would suffice for basic information processing (input, computation, output), but mammals evolved six, providing the architectural capacity for self-modeling --- the ability to process the world \textit{and} observe yourself doing it. Reptiles have three or four cortical layers and lack the rich self-awareness that mammals display. The doubling from three to six layers may have been the evolutionary event that made consciousness possible in mammals (Chapter 2).

\textbf{Neuron.}
The basic cell of the nervous system. Each neuron receives input through its dendrites, integrates that input in its cell body, and (if the integrated signal crosses a threshold) generates an action potential that travels down its axon to synapses with other neurons. The human brain contains roughly 86 billion neurons, each connected to thousands of others. A single neuron is not conscious --- it is a component of the substrate, operating at the electrochemical level. Consciousness emerges from the collective dynamics of billions of neurons organized into the four-model architecture and operating at criticality. The neuron is to consciousness what a transistor is to a video game: necessary but not, by itself, sufficient.

\textbf{Neurotransmitter.}
A chemical messenger released at a synapse when an action potential arrives. The neurotransmitter crosses the tiny synaptic gap and binds to receptors on the receiving neuron, either exciting it (making it more likely to fire) or inhibiting it (making it less likely). The major neurotransmitters mentioned in this book are: glutamate (excitatory), GABA (inhibitory), serotonin (modulatory, targeted by classic psychedelics at the 2A receptor), and dopamine (involved in reward, motivation, and motor control). Neurotransmitters operate at the electrochemical level of the five-level hierarchy --- they change how neurons talk to each other, and their effects ripple upward through the proteomic and topological levels to alter the simulation at the virtual level (Chapter 6).

\textbf{PCI (Perturbational Complexity Index).}
A measure of brain complexity developed by Marcello Massimini and colleagues. The procedure is straightforward: deliver a magnetic pulse to the cortex (via TMS) and measure how complex the resulting brain response is using EEG. In conscious states, the pulse produces a complex, differentiated, far-traveling response (high PCI). In unconscious states --- deep sleep, propofol anesthesia --- the response is simple and stereotyped (low PCI). PCI is the closest thing we have to a consciousness meter, and it maps directly onto the theory's criticality requirement: high PCI corresponds to the brain operating at or near the edge of chaos, low PCI to the brain in a subcritical regime where the simulation cannot be sustained (Chapter 7).

\textbf{Proprioception.}
The sense of your body's position and movement in space --- where your limbs are, how they are oriented, whether they are moving. Proprioceptive sensors in your muscles, tendons, and joints continuously feed information to both the Implicit Self Model (which maintains your body schema) and the Explicit Self Model (which generates your felt sense of having and inhabiting a body). Proprioception is the signal that salvia divinorum disrupts most dramatically: when proprioceptive input is overwhelmed or cut off, the ESM loses its anchor and latches onto whatever other input dominates --- which is how you ``become'' a chair or a wall (Chapter 6). It is also the signal that the rubber hand illusion manipulates to trick the ESM into adopting a fake hand as its own (Chapter 3).

\textbf{Pulvinar.}
The largest nucleus of the thalamus, involved in visual attention and in the subcortical visual pathway that bypasses the cortex entirely. The pulvinar is part of the fast route from retina to superior colliculus to thalamus that gives blindsight patients their paradoxical ability: visual information travels through this pathway to the Implicit World Model, guiding navigation and obstacle avoidance, even though it never reaches the Explicit World Model and therefore never enters conscious experience (Chapter 7).

\textbf{Serotonin 2A receptors.}
The specific receptor subtype targeted by classic psychedelics --- LSD, psilocybin, DMT, and mescaline all bind here. Serotonin 2A receptors are concentrated in the cortex, particularly in the frontal and temporal regions. When psychedelics activate them, the result is a global increase in implicit--explicit permeability: information that is normally processed entirely on the real side --- invisible to consciousness --- begins leaking through to the simulation. This is the mechanism behind the dose-dependent visual progression from phosphenes to geometric patterns to full scenes (Chapter 6). The 2A receptor acts at the electrochemical level, but its effects cascade upward through every subsequent level of the hierarchy.

\textbf{Superior colliculus.}
A structure in the midbrain (not the cortex) involved in eye movements and in the fast, evolutionarily ancient visual pathway that processes spatial information without involving the cortex. The superior colliculus receives direct input from the retina and routes visual information through the pulvinar to motor and spatial processing areas. It is the key structure in blindsight: when the visual cortex is destroyed, the superior colliculus pathway keeps working, feeding spatial information to the Implicit World Model. The patient navigates obstacles flawlessly --- while genuinely experiencing total blindness, because the Explicit World Model receives no visual input (Chapter 7).

\textbf{Synapse.}
The junction between two neurons where signals are transmitted. When an action potential reaches the end of an axon, it triggers the release of neurotransmitter molecules into the tiny gap (the synaptic cleft) between the sending and receiving neurons. The neurotransmitter binds to receptors on the receiving neuron, either exciting or inhibiting it. The synapse is where the electrochemical level meets the proteomic level: the \textit{activity} of the synapse is electrochemical (neurotransmitter release and binding), but the \textit{strength} of the synapse is proteomic (determined by receptor density, molecular scaffolding, and structural factors that change with learning). The human brain contains roughly 100 trillion synapses.

\textbf{Synaptic weights.}
The strengths of the connections between neurons --- how much influence one neuron's firing has on another. Synaptic weights are the physical medium of learning: every time you learn something, synaptic weights change. Your entire Implicit World Model and Implicit Self Model are stored in these weights. They are the proteomic level of the five-level hierarchy (Chapter 2) --- the ``hard drive'' on which the brain's knowledge is written. The key insight is that synaptic weights are never conscious. You cannot introspect on your own synaptic connections. The implicit models stored in those weights generate the simulation, but they are not part of the simulation. This is why the real side is ``lights off.''

\textbf{Thalamus.}
Often called the brain's relay station, the thalamus is a pair of egg-shaped structures near the center of the brain that routes nearly all sensory information to the appropriate cortical regions. Almost nothing reaches the cortex without passing through the thalamus first (the notable exception is olfaction, which has a direct pathway). The thalamus also plays a crucial role in sustaining the cortical automaton: thalamocortical loops --- the back-and-forth signaling between thalamus and cortex --- help maintain the oscillatory dynamics that keep the brain at criticality. When thalamocortical connectivity is disrupted (as by propofol), the cortical automaton collapses into subcritical dynamics and consciousness vanishes (Chapter 7).

\textbf{V1, V2, V3 (visual areas).}
Sequential processing stages in the visual cortex, each handling increasingly complex aspects of vision. V1 (also Brodmann area 17) is the first cortical stop for visual information --- it processes basic features like edges, orientations, and simple contrasts. V2 extracts more complex features like contours and textures. V3 and higher areas handle motion, depth, color, object recognition, and face processing. Under normal conditions, you are conscious only of the final product --- the integrated visual scene in your Explicit World Model --- not of the intermediate representations in V1, V2, or V3. But psychedelics change this: by increasing permeability, they expose these intermediate stages to consciousness in order, from simple (V1 phosphenes and breathing surfaces) to complex (V3+ faces and scenes), in the dose-dependent progression described in Chapter 6.

\textbf{Visual cortex.}
The region at the back of the brain (the occipital lobe) that processes visual information. It contains V1 through V5 and numerous higher visual areas, forming a hierarchy that transforms raw retinal signals into the rich visual world you experience. The visual cortex is the most studied part of the brain, and for good reason: it provides the clearest examples of how implicit processing (feature detection, pattern recognition, object classification) generates explicit experience (the visual scene you see). Damage to the visual cortex produces blindness --- but \textit{which kind} of blindness depends on which part of the architecture is affected. Destroy V1 and you get blindsight: implicit visual processing continues, but the simulation contains no vision. Leave V1 intact but destroy the input pathways and you may get Anton's syndrome: no visual input arrives, but the simulation generates vision from stored models anyway. Blindsight and Anton's syndrome are the theory's most striking mirror pair (Chapter 7).


\chapter{Appendix B: The Intelligence Model}

\textit{This appendix summarizes the recursive intelligence model developed in a companion paper (Gruber, 2026, ``The Recursive Nature of Intelligence''). The full academic treatment is available separately.}

[TODO: Condense from paper/intelligence/paper.md. Key framework elements:]

\begin{itemize}
  \item \textbf{Intelligence as recursive loop}: Knowledge, Performance, and Motivation as three interconnected components that feed into each other.
  \item \textbf{Knowledge types}: Factual, Operational, Social --- each playing a distinct role in the loop.
  \item \textbf{External dependency}: The loop requires external fuel (information, challenges, feedback) to keep iterating.
  \item \textbf{Connection to consciousness}: The four-model architecture provides the \textit{substrate} for intelligence. Intelligence is what the conscious system \textit{does} with its models --- the recursive process of acquiring knowledge, applying it, and being motivated to acquire more.
  \item \textbf{Why IQ tests fail}: They measure a snapshot of one component (performance on specific tasks) rather than the dynamic, recursive, multi-component process that intelligence actually is.
  \item \textbf{Animal intelligence}: Maps onto the consciousness gradient from Chapter 9 --- more sophisticated self-models enable more sophisticated recursive loops.
\end{itemize}
