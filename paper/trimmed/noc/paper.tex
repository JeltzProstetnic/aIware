...
Combined with a criticality requirement derived independently from Wolfram's computational framework \citep{Wolfram2002}, the theory generates nine novel testable predictions and unifies phenomena across psychopharmacology, clinical neurology, sleep science, and comparative cognition under three principles.
...
\textit{Alt text: A 2$\times$2 grid diagram. The vertical axis represents mode (implicit/learned at top, explicit/generated at bottom); the horizontal axis represents scope (world on left, self on right). Top row: Implicit World Model (IWM) containing synaptic weights, causal models, motor programs; Implicit Self Model (ISM) containing body schema, habits, autobiographical memory. A horizontal dashed line labeled ``implicit-explicit permeability boundary'' separates the rows. Bottom row: Explicit World Model (EWM) containing sensory experience, colors, sounds, spatial structure, labeled ``virtual qualia''; Explicit Self Model (ESM) containing the conscious self, sense of agency, marked with a self-referential closure icon. Arrows labeled ``generates'' flow from each implicit model to its explicit counterpart. The implicit side is labeled ``real side: non-conscious''; the explicit side is labeled ``virtual side: phenomenal.''}}
...
\textit{Alt text: Two large circles side by side. The left circle (blue) is labeled ``virtual, simulated, explicit'' and contains the EWM and ESM with ``simulation'' between them; below it reads ``lights on, phenomenal experience.'' The right circle (gray) is labeled ``real, learned, implicit'' and contains the IWM and ISM; below it reads ``lights off, no phenomenal character.'' Arrows labeled ``generates'' connect each implicit model to its explicit counterpart. At the bottom, a rectangle labeled ``real world (external environment)'' has arrows for ``learning'' (into real side) and ``sensory input'' (into virtual side).}}
...
\textit{Alt text: A line graph showing phenomenological content varying over a typical morning. The x-axis marks events: alarm, getting ready, routine driving, a threat (speed trap, highest peak), arriving at work, a social salience spike (attractive colleague, moderate peak), and settling at desk. The y-axis represents phenomenological content. The blue line traces peaks at salient events (threats, social signals) and troughs during routine activities, illustrating that consciousness tracks what matters rather than everything.}}
...
\textbf{3. Physical implementation.} Which physical mechanism in the biological brain supports criticality? Candidates include cortical column dynamics, thalamocortical standing waves, glial modulation, and (more speculatively) quantum processes in microtubules \citep{HameroffPenrose1996}, though see \citet{Tegmark2000} for decoherence objections. The theory is agnostic: it specifies the \emph{functional} requirements without mandating a specific physical mechanism.
...
Nine novel testable predictions include that ego dissolution content is controllable via sensory input (Prediction~3), that psychedelics should alleviate anosognosia (Prediction~4), and that all consciousness-abolishing anesthetics converge on criticality disruption (Prediction~5). The criticality requirement, derived from Wolfram's framework in 2015, converges with the later empirical consolidation \citep{HengenShew2025,AlgomShriki2026}.
