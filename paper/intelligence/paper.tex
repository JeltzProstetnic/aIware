...
This paper proposes that intelligence---understood as \emph{learning ability} (Lernf\"ahigkeit)---is constituted by three interacting components. This recursive model was first proposed in \citet{Gruber2015} and is developed here in its first systematic English-language presentation, with significant theoretical extensions and engagement with the contemporary intelligence literature. The three components are:
...
Population-level evidence supports this process view. \citet{BratsbergRogeberg2018}, analyzing Norwegian military conscript data, demonstrated that IQ scores rose and then declined across birth cohorts \emph{within families}---ruling out genetic explanations and confirming environmental causation. The Flynn effect reversal, now documented across multiple countries, is incompatible with models that treat intelligence as a primarily biological trait. It is, however, precisely what a recursive model predicts: when environmental conditions that support the loop (educational quality, intellectual engagement) degrade, the loop weakens at the population level.

Even more striking is the ``Austrian paradox'' reported by \citet{GignacZajenkowski2024}: IQ scores rose while \emph{g}---the general factor extracted from the correlation matrix---simultaneously declined. This dissociation is inexplicable under static-trait models but directly predicted by the recursive framework. Teaching to the test inflates Performance scores (narrow task-specific knowledge) without engaging the recursive loop. The result is higher scores on standardized measures with lower capacity for the self-directed, generalizable learning that the recursive loop produces. IQ goes up; intelligence, properly understood, goes down.
...
\bibitem[Balboni et~al., 2021]{Balboni2021}
Balboni, G., Ferrandi, A., \& Naglieri, J.~A. (2021).
\newblock Adaptive intelligence: Intelligence is not a personal trait but emerges from person $\times$ task $\times$ situation interaction.
...
\bibitem[Melby-Lerv\aa{}g \& Hulme, 2013]{MelbyLervag2013}
