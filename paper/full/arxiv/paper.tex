...
Recent developments have deepened the crisis rather than resolving it. The COGITATE adversarial collaboration---whose protocol was pre-registered by \citet{Melloni2023} and whose results were published in \textit{Nature} by the \citet{COGITATE2025}---produced equivocal results: neither IIT nor GNW was fully confirmed, and the data favored posterior cortical involvement over either camp's predictions. A letter signed by over 100 researchers declared IIT pseudoscientific \citep{IITConcerned2025,NatNeuro2025IIT}, provoking fierce rebuttals \citep{TononiAlbantakis2025} and calls for more nuanced framing \citep{GomezMarin2025}. Reviews published in 2024--2025 have asked whether the field is making genuine progress or merely accumulating incompatible frameworks.
...
Two paths---theoretical reasoning from Wolfram's computational universality framework \citep{Gruber2015} and large-scale empirical neuroscience \citep{HengenShew2025,AlgomShriki2026}---converging on the same claim. Although the empirical criticality program was already underway \citep{Beggs2003} when \citet{Gruber2015} derived the requirement from computational first principles rather than from neuroimaging data, this convergence does not prove the Four-Model Theory correct, but it provides notable support for one of its core predictions.
...
$^\dagger$~Axiomatic identification of consciousness with $\Phi$; whether this constitutes a solution or a redefinition is debated.
$^{\dagger\dagger}$~IIT's panpsychist commitments lead to the Combination Problem \citep{Chalmers2016}, which remains unresolved.
...
\textbf{Unique?}: \textbf{Yes---few competing theories generate this specific prediction,} though predictive processing frameworks might produce a related account through the breakdown of self-related priors. IIT, GNW, HOT, and AST have no mechanism for specifying what a subject will ``become'' during ego dissolution. This is the theory's most distinctive empirical prediction.
...
\item \textbf{Lucid dreaming as ESM threshold crossing}: During REM, if the substrate reaches sufficient criticality, the ESM activates---producing lucid dreaming. This onset should be detectable as a step-like discontinuity in EEG complexity measures, not a gradual ramp (\citealp{LaBerge1985}).
...
\textbf{7. The holographic-automaton conjecture.} Section~5.2 describes the implicit models as storing information holographically---in distributed, non-local representations across the substrate. Section~3.7 requires the substrate to operate at criticality (Class~4 dynamics). These two properties---holographic information storage and Class~4 computation---raise three open mathematical questions of increasing ambition: (a)~Does there exist a Class~4 cellular automaton whose emergent behavior produces holographic patterns---that is, whose global dynamics encode information in a distributed, non-local manner that degrades gracefully under partition? This would mean that holographic storage is not merely a feature of trained neural networks but an intrinsic consequence of critical dynamics. (b)~Does there exist a Class~4 cellular automaton whose \textit{rule structure} is itself holographic---where the rules governing local interactions are a compressed encoding of a global pattern? (c)~Does there exist a Class~4 cellular automaton satisfying both (a) and (b): holographic rules generating holographic output through Class~4 dynamics? Such a system would constitute a computational fixed point---a process that encodes its own structure at every level. These questions are speculative and lie at the boundary of the theory's current scope, but they are well-defined mathematical problems. If (c) has a positive answer, it would suggest a deep connection between critical computation, holographic encoding, and self-referential structure---potentially relevant to cosmological questions about the computational nature of physical law.
...
The theory's criticality requirement was derived from Wolfram's computational framework in 2015 \citep{Gruber2015}---independently of, though not prior to, the empirical criticality program initiated by \citet{Beggs2003}---and the same conclusion was subsequently consolidated through \citeauthor{HengenShew2025}'s \citeyearpar{HengenShew2025} meta-analysis of 140 datasets and \citeauthor{AlgomShriki2026}'s \citeyearpar{AlgomShriki2026} ConCrit framework. This convergence from a theoretical derivation and large-scale empirical synthesis provides notable support.
