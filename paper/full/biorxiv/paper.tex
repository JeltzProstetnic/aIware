...
\textbf{The virtual side} (EWM + ESM): These are simulated, transient, generated, and phenomenal. They are patterns of activity---\hspace{0pt}in biological brains, transient electrochemical dynamics. They are constructed in real time from the implicit models and current sensory input. They \emph{are} experience. ``Lights on.''
...
\textbf{The dissolution}: The physical processing \emph{does not} feel like anything. The IWM and ISM---\hspace{0pt}the substrate-level implicit models---\hspace{0pt}operate without any phenomenal character whatsoever. There is nothing it is like to be a synaptic weight. The simulation, however, \emph{does} feel---\hspace{0pt}and within the simulation, qualia are simply what self-perception produces. Asking why neuronal firing feels like something is analogous to asking why transistor switching feels like running a video game. The transistors do not run the game at the level of individual switching; the virtual machine does. The neurons do not experience redness at the level of individual firing; the simulation does, and within the simulation, ``redness'' is simply the ESM's mode of registering a particular class of EWM content.

\textbf{Why self-simulation specifically?} A critic might object that this merely relocates the Hard Problem: why does \emph{this} virtual process have experience when a weather simulation does not? The answer lies in \textbf{self-referential closure}. A weather simulation models weather; it does not model \emph{itself modeling weather}. The four-model architecture creates a closed loop: the ESM is the system modeling its own modeling process. In this loop, the distinction between the model and the modeled collapses---\hspace{0pt}the simulation \emph{is} the thing being simulated. Qualia are not an \emph{addition} to the self-modeling; they are the self-modeling as encountered from the inside of the loop. A non-self-referential simulation has an outside from which it can be described without remainder; a self-referential simulation at criticality has no such outside. The simulation \emph{is} its own observer, and observation-from-inside is what we call experience.

This is not a proof that self-referential simulation must be conscious---\hspace{0pt}it is an argument that self-referential simulation is the \emph{kind} of process for which the Hard Problem's assumptions break down. Self-referential closure is precisely the condition under which the gap between process and feeling does not exist.
...
\subsection{3.7.1 The Five-System Hierarchy}
...
\item \textbf{Virtual system}: The real-time dynamical pattern---\hspace{0pt}the cortical automaton (Section~3.7.2)---\hspace{0pt}that constitutes the explicit models (EWM, ESM). This is the level at which consciousness exists.
\end{enumerate}
...
\subsection{3.7.2 The Cortical Automaton}
...
\subsection{3.7.3 Two Thresholds for Consciousness}
...
\textbf{Lucid dreaming} provides direct evidence for the software-like quality of the virtual models. In a lucid dream, the dreamer becomes aware that they are dreaming: the ESM ``toggles on'' more fully, gaining metacognitive access within the dream state. The Four-Model Theory predicts that lucid dream onset corresponds to a \textbf{criticality threshold crossing}---\hspace{0pt}a step-like increase in neural complexity as the ESM activates more fully. This should be detectable as a discontinuity in EEG complexity measures at the moment of lucid dream onset (see Section~8, Prediction 8).
...
Ratings: $\bullet$ = addresses, $\odot$ = partial, $\circ$ = minimal,---\hspace{0pt}= silent, n/a = not applicable.


† IIT addresses the Hard Problem through its axioms, identifying consciousness with integrated information ($\Phi$). Whether this constitutes a solution or a redefinition is debated (see §7.2).
†† IIT's panpsychist commitments lead to the Combination Problem \citep{Chalmers2016}, which remains unresolved within the framework.
\* GNW and PP proponents argue these theories address the ``real problem'' of consciousness \citep{Seth2021}---\hspace{0pt}explaining the structure and contents of experience---\hspace{0pt}even if they do not address the Hard Problem as Chalmers defines it. This is a legitimate methodological choice, not a deficiency; the ``silent'' rating reflects the scope of the requirement as defined in §2, not a judgment on the theories' overall merit.
...
\textbf{Unique?}: Yes in the specific form. While individual contrasts (e.g., self vs. world processing) are well-studied, the Four-Model Theory predicts a specific 2$\times$2 factorial structure in neural activation that no other theory mandates. This prediction is placed first because it most directly tests the four-model architecture itself---\hspace{0pt}the central structural claim of the theory.
...
\textbf{Unique?}: \textbf{Yes---\hspace{0pt}few competing theories generate this specific prediction.} IIT, GNW, HOT, and AST have no mechanism for specifying what a subject will ``become'' during ego dissolution. Predictive processing frameworks might produce a related account through the breakdown of self-related priors, but do not predict the specific input-tracking pattern. This is the theory's most distinctive empirical prediction.
...
\textbf{Unique?}: \textbf{Yes---\hspace{0pt}this is a cross-domain surprise prediction.} No other theory connects psychedelics and anosognosia through a single mechanism. Confirmation would be strong evidence for the variable-permeability principle.


\subsection{Prediction 5: All Anesthetics Converge on Criticality Disruption}

\textbf{Statement}: Despite diverse receptor-level mechanisms (GABAergic, NMDA, opioid, $\alpha$2-adrenergic), all agents that abolish consciousness do so by pushing the substrate below the criticality threshold. Agents that alter but do not abolish consciousness (ketamine, low-dose psychedelics) do not push below criticality.
...
\textbf{Statement}: After callosotomy, each hemisphere retains a degraded but functionally \emph{complete} set of cognitive and experiential capacities---\hspace{0pt}not a clean hemispheric specialization. The degradation should be proportional to the extent of commissural severing (partial callosotomy $\to$ partial degradation).
...
\textbf{Statement}: Sleep is the substrate's mechanism for restoring the conditions that support consciousness. The brain's analog substrate is inherently unstable---\hspace{0pt}never truly calibratable for sustained digital computation. Consciousness emerges as a self-organizing cellular automaton (CA) at criticality, providing a stable digital layer on top of this drifting substrate. The CA is stable for extended periods (waking), but the underlying biochemical substrate slowly drifts (neurotransmitter depletion, metabolic waste accumulation) until it can no longer sustain the CA. At that point, the CA breaks down radically---\hspace{0pt}not gradually---\hspace{0pt}producing sleep onset. NREM sleep restores the substrate; as it periodically re-approaches the criticality threshold during restoration, the CA briefly re-emerges, producing REM sleep and dreams. The 90-minute ultradian cycle is the substrate oscillating around the critical point during this restoration process.
...
\item \textbf{Sleep onset as radical transition}: The transition to sleep should correspond to a step-like drop in criticality markers, not a gradual dimming---\hspace{0pt}reflecting the CA's digital breakdown.
...
\item \textbf{Lucid dreaming as ESM threshold crossing}: During REM, if the substrate reaches sufficient criticality, the ESM activates---\hspace{0pt}producing lucid dreaming. This onset should be detectable as a step-like discontinuity in EEG complexity measures, not a gradual ramp \citep{LaBerge1985}.
...
\textbf{Unique?}: Partially shared with criticality frameworks in general, but the specific claim---\hspace{0pt}that sleep \emph{exists because} the CA requires periodic substrate restoration, and that NREM/REM architecture directly reflects criticality oscillations---\hspace{0pt}is distinctive. No competing theory of consciousness provides this specific functional account of sleep architecture.


\subsection{Prediction 9: DID Alters Have Distinct Neural Process Signatures}

\textbf{Statement}: Different alters in DID correspond to distinct, measurable configurations of neural activity---\hspace{0pt}not merely behavioral differences or different self-reports, but different neural dynamics detectable through neuroimaging.
...
(c) \textbf{Class 4 dynamics $\to$ holographic emergent behavior.} Does a Class 4 automaton, regardless of its rule structure, necessarily produce holographic-like properties in its emergent patterns---\hspace{0pt}distributed information, graceful degradation, part-contains-whole structure? If this can be demonstrated, the holographic character of neural information storage would be derivable from the criticality requirement alone. A system satisfying both (a) and (b)---\hspace{0pt}holographic rules generating holographic output through Class 4 dynamics---\hspace{0pt}would constitute a computational fixed point: a process that encodes its own structure at every level. If such a system exists, it would suggest a deep connection between critical computation, holographic encoding, and self-referential structure---\hspace{0pt}potentially relevant to cosmological questions about the computational nature of physical law.
...
\begin{enumerate}
\item \textbf{Test the criticality prediction across all anesthetic agents} (Prediction 5)---\hspace{0pt}this is achievable with current methods and would provide strong evidence for or against the criticality framework.
\item \textbf{Design controlled ego-dissolution experiments} (Prediction 3)---\hspace{0pt}the most distinctive prediction, uniquely generated by the redirectable ESM mechanism.
\item \textbf{Investigate the psychedelic-anosognosia connection} (Prediction 4)---\hspace{0pt}a cross-domain prediction that, if confirmed, would constitute strong evidence for the variable-permeability mechanism.
\item \textbf{Measure criticality at lucid dream onset} (Prediction 8)---\hspace{0pt}achievable with established paradigms and equipment.
\end{enumerate}
